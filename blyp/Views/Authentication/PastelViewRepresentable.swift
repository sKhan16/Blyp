...
struct PastelViewRepresentable: UIViewRepresentable {
    ...
    func makeUIView(context: Context) -> PastelView {
        let pastel = PastelView(frame: UIScreen.main.bounds)
        pastel.setColors(self.colors)
        pastel.startPastelPoint = self.startPastelPoint
        pastel.endPastelPoint = self.endPastelPoint
        pastel.animationDuration = self.animationDuration
        pastel.startAnimation()
        return pastel
    }
    
    func updateUIView(_ uiView: PastelView, context: Context) {}
}
